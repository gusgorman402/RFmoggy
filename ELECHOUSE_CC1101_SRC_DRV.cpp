...
void ELECHOUSE_CC1101::SendData(byte *txBuffer,byte size)
{
  ...
  SpiStrobe(CC1101_STX); //start send  
   //interrupt driven method for knowing when transmission is finished Section15.6 in datasheet
    while (!digitalRead(GDO0));               // Wait for GDO0 to be set -> sync transmitted  
    while (digitalRead(GDO0));                // Wait for GDO0 to be cleared -> end of packet
  //SpiStrobe(CC1101_SFTX); //flush TXfifo

  //SPI polling method to know when transmission finished, back to IDLE. Unless registers set to stay in TX(less delay, TX to IDLE is 75microseconds Section19.6 and MCSM1 register)
  while(marcstate != 0x01){
      marcstate = (SpiReadStatus(CC1101_MARCSTATE) & 0x1F);
  }
