...
void handleJukeboxSend() {  
  ...
  Serial.println(); Serial.print("TX: Jukebox "); Serial.println(button); Serial.println("----------");
  
  if( server.arg("remotebutton").equals("freeCredit") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_freeCredit, false); }
  if( server.arg("remotebutton").equals("pauseSong") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_pauseSong, false); }
  if( server.arg("remotebutton").equals("skipSong") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_skipSong, false); }
  if( server.arg("remotebutton").equals("volumeUp") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_volumeUp, false); }
  if( server.arg("remotebutton").equals("volumeDown") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_volumeDown, false); }
  if( server.arg("remotebutton").equals("powerOff") ){ binaryBruteSend(bruteSize, "10", "1000", 433.92, 1.77777, 2, 50, 500, jukeboxID, false, jukebox_pwm_powerOff, false); }

  Serial.println("----------"); Serial.print("EOTX: Jukebox "); Serial.println(button); Serial.println();
...
void handleGarageSend() {  
  ...
  if( server.arg("chamber7").equals("true") ){ 
    Serial.println(); Serial.println("TX: Chamberlain7 Brute"); Serial.println("----------");
    binaryBruteSend(7, "0111", "0011", 390, 1.0, 8, 40, 100, "", false, "", false);
    Serial.println("----------"); Serial.println("EOTX: Chamberlain7 Brute"); Serial.println();
  }
  if( server.arg("linear8").equals("true") ){
    Serial.println(); Serial.println("TX: Linear8 Brute"); Serial.println("----------");
    binaryBruteSend(8, "10000000", "11110000", 310, 2.0, 8, 40, 100,  "", false, "", false);
    Serial.println("----------"); Serial.println("EOTX: Linear8 Brute"); Serial.println();
  }
  if( server.arg("chamber8").equals("true") ){
    Serial.println(); Serial.println("TX: Chamberlain8 Brute"); Serial.println("----------");
    binaryBruteSend(8, "0111", "0011", 390, 1.0, 8, 40, 100, "", false, "", false); 
    Serial.println("----------"); Serial.println("EOTX: Chamberlain8 Brute"); Serial.println();
  }
  if( server.arg("chamber9").equals("true") ){
    Serial.println(); Serial.println("TX: Chamberlain9 Brute"); Serial.println("----------");
    binaryBruteSend(9, "0111", "0011", 390, 1.0, 8, 40, 100,  "", false, "", false);
    Serial.println("----------"); Serial.println("EOTX: Chamberlain9 Brute"); Serial.println();
  }
  if( server.arg("stanley10").equals("true") ){
    Serial.println(); Serial.println("TX: Stanley10 Brute"); Serial.println("----------");
    binaryBruteSend(10, "1000", "1110", 310, 2.0, 8, 40, 100,  "", false, "", false);
    Serial.println("----------"); Serial.println("EOTX: Stanley10 Brute"); Serial.println();
  } 
  if( server.arg("linear10").equals("true") ){
    Serial.println(); Serial.println("TX: Linear10 Brute"); Serial.println("----------");
    binaryBruteSend(10, "1000", "1110", 300, 2.0, 8, 40, 100,  "", false, "", false);
    Serial.println("----------"); Serial.println("EOTX: Linear10 Brute"); Serial.println();
  }
...
void handleDebruijnSend(){
  ...
  if( server.arg("linearDebruijn").equals("true") ){
    Serial.println(); Serial.println("TX: Linear10 DeBruijn"); Serial.println("----------");
    binaryBruteSend(0, "1000", "1110", 300, 2.0, repeatDebru, 100, 200,  "", false, debruijn_ten, true);
    Serial.println("----------"); Serial.println("EOTX: Linear10 DeBruijn"); Serial.println();
  }
  if( server.arg("stanleyDebruijn").equals("true") ){
    Serial.println(); Serial.println("TX: Stanley10 DeBruijn"); Serial.println("----------");
    binaryBruteSend(0, "1000", "1110", 310, 2.0, repeatDebru, 100, 200,  "", false, debruijn_ten, true);
    Serial.println("----------"); Serial.println("EOTX: Stanley10 DeBruijn"); Serial.println();
  }
  if( server.arg("chamberDebruijn").equals("true") ){
    Serial.println(); Serial.println("TX: Chamberlain789 DeBruijn"); Serial.println("----------");
    binaryBruteSend(0, "0111", "0011", 390, 1.0, repeatDebru, 100, 200,  "", false, debruijn_nine, true);
    Serial.println("----------"); Serial.println("EOTX: Chamberlain789 DeBruijn"); Serial.println();
  }
  if( server.arg("mooreDebruijn").equals("true") ){
    Serial.println(); Serial.println("TX: Linear8 DeBruijn"); Serial.println("----------");
    binaryBruteSend(0, "10000000", "11110000", 310, 2.0, repeatDebru, 100, 200,  "", false, debruijn_eight, true);
    Serial.println("----------"); Serial.println("EOTX: Linear8 DeBruijn"); Serial.println();
  }
...
void handleRfpwnonSend(){
  ...
  Serial.println(); Serial.println("TX: RfPwnOn Signal"); Serial.println("----------");
  ...
  Serial.println("----------"); Serial.println("EOTX: RfPwnOn Signal"); Serial.println();
...
void handleJammerSend(){
  ...
  Serial.println(); Serial.println("TX: Jammer Signal"); Serial.println("----------");
  binaryBruteSend(0, "0", "1", jamMHz, 2.0, repeatJam, 1, 1,  "", false, jam_packet, false);  
  Serial.println("----------"); Serial.println("EOTX: Jammer Signal"); Serial.println();
  ...
  for(int x = 0; x < highestDIP; x++){
    ...
    for( int k = 0; k <= pulseReps; k++ ){
      ...
      for( int h = 0; h < sendSections; h++){
        ...
        //delay(1); //sending multiple full packets with no delay will crash sometimes. Power issue??
      }
      memset(sendPacket, 0, 60);
...
void setup(void){
  ...
  ELECHOUSE_cc1101.setClb(1,16,18);
